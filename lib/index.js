...
function mkDirByPathSync(targetDir, onSuccess, onError) {
  var sep = _path2.default.sep;
  var baseDir = process.cwd();
  var dirsToCreate = targetDir.split(process.cwd())[1].substr(1);
  var dirsList = dirsToCreate.split(sep);

  dirsList.reduce(function (parentDir, childDir, i) {
...
function writeCriticalFile(filePath, css) {
  var preventLoop = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : false;
  ...
  _fs2.default.writeFile(filePath, css, { flag: append ? 'a' : 'w' }, function (err) {
    ...
    if (err) {
      if (err.code === 'ENOENT' || err.errno === -4058) {
        mkDirByPathSync(filePath, function () {
...
function handleCriticalFileCreationError(err) {
  console.error(err);
  process.exit(1);
}
